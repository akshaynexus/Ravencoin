...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->GetBlocksToMaturity() > 0)
                continue;
            // bool isAssetScript = pcoin->tx->isasset
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nTime, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key, CBlockTemplate *pblocktemplate)
{
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        ...
        for (unsigned int n=0; n < std::min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBestHeader; n++)
        {
            boost::this_thread::interruption_point();
            ...
            if (CheckKernel(pindexPrev, nBits, nTime - n, prevoutStake, *pcoinsTip, stakeCache))
            {
                // Found a kernel
    ...
    // Calculate reward
    {
        ...
        int64_t nReward = nFees + GetBlockSubsidy(nHeight, GetParams().GetConsensus());
        if (nReward < 0) {
           return false;
        }
