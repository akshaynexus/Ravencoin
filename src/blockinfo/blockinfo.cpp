...
CTxOut GetStakeTXOut(const CTxIn& txin)
{
    ...
    uint256 hashBlock;
    if (GetTransaction(txin.prevout.hash, prevTx, GetParams().GetConsensus(), hashBlock, true)) {
...
std::string GetBlockRewardWinner(const CBlock& block)
{
    ...
    if(txout != CTxOut() && ExtractDestination(txout.scriptPubKey, dstAddr))
        rewardWinner = EncodeDestination(dstAddr);
    return rewardWinner;
}
...
CAmount GetCoinbaseReward(const CBlock& block)
{
    ...
    if(block.IsProofOfStake())
        BlockReward = coinbaseTX->GetValueOut(AreEnforcedValuesDeployed()) - GetTXInputAmount(coinbaseTX->vin);
    else
