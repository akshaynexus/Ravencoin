...
{
      if ((nTimeTx < nBlockTime) && !(txPrev->nHeight <= GetParams().GetConsensus().nFirstPOSBlock))  // Transaction timestamp violation
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBlockTime, unsigned int nBits, CValidationState &state, CCoinsViewCache& view)
{
    ...
    if (!GetTransaction(txin.prevout.hash, txPrev, GetParams().GetConsensus(), hashBlock, true) || !GetStakeCoin(txin.prevout, coinTxPrev, blockTxFrom, pindexPrev, state, view))
